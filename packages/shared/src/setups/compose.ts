 ...
 *
 * This is sound because every category pool in MVP is faction-homogeneous:
 * `RANDOM_TOWN` always yields a Town role and `RANDOM_MAFIA` always yields a
 * Mafia role. The faction count of a setup is therefore determined statically.
 */
...
export function slotFaction(slot: SetupSlot): Faction {
  ...
  return slot.category === 'RANDOM_TOWN' ? 'TOWN' : 'MAFIA';
}
