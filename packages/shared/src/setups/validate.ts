...
function slotIsKilling(slot: SetupSlot, townPool: readonly RoleId[]): boolean {
  if (slot.kind === 'fixed') {
    ...
    // Any MAFIA or TRIAD slot carries a standing faction kill; a VAMPIRE or CULT
    // slot carries a standing CONVERSION that drives the game to a parity win (it
    // grows the faction until it ends the game, so the game can always reach a
    ...
    return (
      ...
      f === 'VAMPIRE' ||
      f === 'CULT' ||
      ROLES[slot.role].nightAction === 'kill'
    );
  }
