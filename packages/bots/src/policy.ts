...
export class BotPolicy {
  ...
  private doNight(): void {
    ...
    // Two-target abilities (Witch control, batch E; Transporter swap, batch F) need
    // a SECOND target distinct from the first. A submission missing either is
    // dropped by the engine, so we only submit when we have both.
    if (ability === 'witch_control' || ability === 'transport') {
      if (target === null) return;
      const self = this.bot.view.seat;
      const second = this.choice([...this.bot.view.alive].filter((s) => s !== self && s !== target));
      if (second === null) return;
      this.bot.send({ v: 1, type: 'night_action', ability, target, target2: second } as ClientMessage);
      return;
    }
