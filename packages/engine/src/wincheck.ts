 ...
 *
 * Generalized for TWO independent evil killing factions — Mafia and Triad. They
 * are enemies, not allies: two evil factions can never co-win, they must wipe
 * each other (and any neutral killer) out first. The rules (see {@link checkWin}):
 *
 ...
 *     there are NO living members of the OTHER evil killing factions (the other
 *     of Mafia/Triad AND no living NK), and F has reached parity with the rest
 *     (|F| >= |living non-F|).
 *   - The Serial Killer (NK) wins iff it is the last killer standing (no living
 *     Mafia and no living Triad), among only itself + benign neutrals.
 *   - If TWO OR MORE killing factions (Mafia / Triad / NK) are alive, the game
 *     CONTINUES — no one wins yet; they fight it out.
 *
 * The 1v1 auto-resolve and the stalemate guard follow the same priority ladder:
 * SK > evil faction (Mafia/Triad) > Town. A pure Mafia-vs-Triad endgame with
 * nothing else alive resolves to the LARGER faction; an exact tie continues (the
 * auto-resolve) / draws-by-largest (the stalemate guard) — see below.
 */
...
function tally(state: GameState): FactionTally {
  const t: FactionTally = { town: [], mafia: [], triad: [], sk: [], benign: [] };
...
export function checkWin(
  ...
  const mafiaAlive = mafiaCount > 0;
  const triadAlive = triadCount > 0;

  // 3. Serial Killer wins if SK alive and it is the last killer: no living Mafia,
  //    no living Triad (only SK + benign neutrals remain).
  if (skAlive && !mafiaAlive && !triadAlive && t.town.length === 0) {
    return { reason: 'serial_killer_last', winners: ['SERIAL_KILLER'] };
  }

  // 1. Town wins: no living Mafia, no living Triad, and no living SK.
  if (!mafiaAlive && !triadAlive && !skAlive) {
  ...
  //    Mafia and Triad are mutually exclusive winners — if both live, neither wins.
  // 2a. Mafia parity: Mafia alive, no Triad, no SK, mafiaCount >= rest.
  if (mafiaAlive && !triadAlive && !skAlive && mafiaCount >= living.length - mafiaCount) {
    return { reason: 'mafia_parity', winners: ['MAFIA'] };
  }
  // 2b. Triad parity: Triad alive, no Mafia, no SK, triadCount >= rest.
  if (triadAlive && !mafiaAlive && !skAlive && triadCount >= living.length - triadCount) {
    return { reason: 'triad_parity', winners: ['TRIAD'] };
  }

  // 4. 1v1 auto-resolve at start of DAY_VOTING: a deadlocked endgame neither side
  //    can mechanically break → priority SK > evil faction > Town.
  if (opts.atDayVotingStart) {
    // SK + exactly one Mafia (no Triad, no Town): SK wins (immune; can't be out-killed).
    if (skAlive && mafiaCount === 1 && !triadAlive && t.town.length === 0) {
      return { reason: 'one_v_one', winners: ['SERIAL_KILLER'] };
    }
    // SK + exactly one Triad (no Mafia, no Town): SK wins.
    if (skAlive && triadCount === 1 && !mafiaAlive && t.town.length === 0) {
      return { reason: 'one_v_one', winners: ['SERIAL_KILLER'] };
    }
    // SK + exactly one Town (no Mafia, no Triad): SK wins (town can't lynch alone).
    if (skAlive && t.town.length === 1 && !mafiaAlive && !triadAlive) {
      return { reason: 'one_v_one', winners: ['SERIAL_KILLER'] };
    }
    // Pure Mafia-vs-Triad endgame (no Town, no SK, no benign): the LARGER killing
    // faction wins; an exact tie cannot be broken (neither can out-kill the other
    // without dying), so the game continues until one side pulls ahead.
    if (mafiaAlive && triadAlive && !skAlive && t.town.length === 0 && t.benign.length === 0) {
      if (mafiaCount > triadCount) return { reason: 'one_v_one', winners: ['MAFIA'] };
      if (triadCount > mafiaCount) return { reason: 'one_v_one', winners: ['TRIAD'] };
      // tie → continue.
    }
    // (Mafia/Triad-vs-Town parity already covered by rules 2a/2b.)
  }
 ...
 * ends — largest living faction wins. Priority ladder for ties: SK > Triad >
 * Mafia > Town (SK counts as a faction of 1 and wins ties; between the two evil
...
export function checkStalemate(
  ...
  // Larger count wins; ties broken by priority (SK > Triad > Mafia > Town).
  ...
  if (t.sk.length > 0) candidates.push({ party: 'SERIAL_KILLER', count: t.sk.length, priority: 3 });
